...
const renderExplodedPlaceholder = resolve(
  pagesDir,
  "_render-placeholder-exploded.svg",
);
...
const designRenderPlaceholder = resolve(
  pagesDir,
...
/**
 * Type into the sidebar filter, then take focus back off the field.
 *
 * The blur is the point of this helper. `fill()` leaves the field focused, and a focused
 ...
 *
 * Blurring makes these three about the one thing they claim — what the filter did to the LIST.
 * The focused field is not left uncovered: `serve-landing-sections-filter-focus` exists for it and
 * WAITS on `document.activeElement`, so it fails loudly rather than drifting a pixel if anything
 * ever really does steal focus from the box.
 */
async function filterTo(page, query) {
  await page.fill("#cp-search", query);
  await page.locator("#cp-search").evaluate((el) => el.blur());
...
const FIXTURE_STATES = [
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.waitForSelector(
        ".cp-page-tip:not([hidden]) .cp-page-tip-card",
      );
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.click(
        '.cp-page-lane label:has([data-cp-page-lane][value="design"])',
      );
      ...
      await page.waitForFunction(
        () =>
          !document
            ...
            .classList.contains("cp-page-hide-design"),
      );
    },
  },
  {
    ...
    apply: async (page) => {
      ...
      const worst = await page.evaluate(() => {
        const values = Array.from(
          document.querySelectorAll(".cp-page-score"),
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.waitForFunction(() =>
        document
          .querySelector(".cp-page-stage")
          .classList.contains("cp-page-outlines-on"),
      );
    },
  },
  {
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.waitForFunction(() => {
        ...
        const level = document.querySelector("[data-cp-page-zoom-level]");
        return (
          bar && !bar.hidden && level && parseInt(level.textContent, 10) > 150
        );
      });
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      const marks = await page.evaluate(() => {
        ...
        const spot = document.querySelector(
          '.cp-page-node[data-cp-node="1:1"]',
        );
        const style = getComputedStyle(spot);
        return {
          zoom: parseFloat(
            getComputedStyle(stage).getPropertyValue("--cp-page-zoom"),
          ),
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.waitForFunction(
        () => document.querySelector("cp-page-zoom").hidden,
      );
    ...
    apply: async (page) => {
      ...
      await page.waitForFunction((id) => {
        ...
        const img = document.querySelector(
          `.cp-page-render[data-cp-node="${id}"]`,
        );
        return (
          !!target &&
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.waitForFunction(() => {
        const themes = document.querySelector('.cp-tab[data-tab="themes"]');
        ...
        return reachable.length > 0 && reachable.some((r) => r.tabIndex === 0);
      });
    },
  },
  {
    ...
    apply: async (page) => {
      ...
      await page
        .waitForFunction(() => {
          ...
          return (
            score && score.textContent && score.textContent !== "comparing…"
          );
        })
        .catch(() => {});
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await page.waitForSelector(".cp-actions-menu[open] + .cp-actions-panel");
      await page.mouse.move(0, 0);
    },
  },
  {
  ...
  },
  {
    ...
    apply: async (page) => {
      ...
      await expect(page.locator(".cp-annotation-entry--theme")).toHaveCount(1);
      ...
      await expect(page.locator(".cp-annotation--typography-hit")).toHaveCount(
        5,
      );
      ...
      await expect(
        page.locator(".cp-annotation--typography-hit.cp-annotation-active"),
      ).toHaveCount(4);
    },
  },
  {
  ...
  for (const theme of listThemes()) {
    test(`snapshot · ${fixture} · ${theme}`, async ({ page }) => {
      ...
      if (fixture === "serve-parity") {
        await page
          .waitForFunction(() => {
            const status = document.getElementById("cp-parity-score-status");
...
test("contract · declared theme renders use bounded parallelism", async ({
  page,
}) => {
  let active = 0;
  let maxActive = 0;
...
test("contract · a page branch opens from the keyboard, and Enter still follows it", async ({
  ...
  const branch = page
    .locator("#cp-pane-pages .cp-page-branch > .cp-tree-page")
    .first();
